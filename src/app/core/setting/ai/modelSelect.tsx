...
export default function ModelSelect(
  ...
  async function getModels(model: AiConfig, requestId: number) {
    try {
      ...
      console.error('[getModels] Failed to fetch models:', err)
  ...
  return (<>
    {list.length ? (
        ...
        <div className="flex gap-2 flex-col">
          ...
          {error && 
            <div className="flex gap-2 items-center text-xs text-destructive">
              <AlertCircle className="size-4" />
              <p className="line-clamp-1 flex-1">{error}</p>
            </div>
          }
        </div>
      }
